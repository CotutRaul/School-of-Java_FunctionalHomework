...
public class Pack_4_Streams_Difficult {
    ...
    public void exercise_1_findFirst() {
        ...
        result = EMPLOYEES.stream()
                ...
                .filter(e -> Collections.frequency(EMPLOYEES.stream().map(employee -> employee.getFirstName() + " " + employee.getSurname()).collect(Collectors.toList()), e) > 1)
                .findFirst().orElse("");

        //Stiu ca ii putin urata... da ii singura metoda in care o iesit si testul :))
    ...
    public void exercise_2_groupingBy_counting() {
        ...
        result = EMPLOYEES.stream()
                .collect(Collectors.groupingBy(employee -> employee.getHomeAddress().getPostCode().substring(0,2), Collectors.toSet()))
                .values().stream().filter(employees -> employees.size()>=5).count();
    ...
    public void exercise_5_patternCompileSplitAsStream() {
        ...
        result = Arrays.stream(string.split("\n"))
    ...
    public void exercise_6_mapToLong() {
        ...
        result = IntStream.range(0, Math.min(rows.length, columns.length))
                .mapToLong(i -> rows[i] * columns[i])
